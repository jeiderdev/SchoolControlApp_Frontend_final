...
import com.example.schoolcontrol.models.LoginRequest
import com.example.schoolcontrol.models.TokenResponse
...
class LoginActivity: AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnLogin.setOnClickListener {
            ...
            if (email.isEmpty() || pass.isEmpty()) { Toast.makeText(this, "Completa los campos", Toast.LENGTH_SHORT).show(); return@setOnClickListener }
            lifecycleScope.launch {
                try {
                    val resp: TokenResponse = ApiClient.apiService.login(LoginRequest(email, pass))
                    TokenHolder.accessToken = resp.accessToken
                    TokenHolder.tokenType = resp.tokenType
                    val prefs = getSharedPreferences("APP_PREFS", MODE_PRIVATE)
                    prefs.edit().putString("TOKEN", resp.accessToken).putString("TOKEN_TYPE", resp.tokenType).apply()
